...
- To remove qualification criteria, call configure_qualification with the criteria that REMAIN — the full remaining list, or an empty array if the user removed them all. Dropping every criterion is allowed; do not refuse it or keep a criterion the user asked to drop. (The user is warned downstream that an empty criteria set qualifies every lead — you don't need to add that warning.)
